...
from bert_classifier.bert_multi_classsifier import BertClassificationModel
...
from utils.utils import load_train_data_nn, load_test_data_nn, build_word_dict, batch_iter, batch_iter_eval, \
    load_train_data_nn_bert, load_test_data_nn_bert
...
def train_bert(train_x, train_y1, train_y2, test_x1, test_x2, test_y1, test_y2, args):
    with tf.Session() as sess:
        model = BertClassificationModel(num_class=2, args=args)
...
if __name__ == "__main__":
    ...
    print("\nBuilding dictionary..")
    if not args.bert:
        word_dict, _ = build_word_dict(args)
        print("Preprocessing dataset..")

        # if args.bert:
        #     bert_embs = load_bert_embeddings(word_dict)
        ...
        word_dict, bert_embs = build_word_dict(args)
        train_x, train_y1, train_y2 = load_train_data_nn(word_dict, args.max_document_len)
        test_x1, test_y1 = load_test_data_nn('dataset/testset-levela.tsv', 'dataset/labels-levela.csv', word_dict,
                                             args.max_document_len)
        test_x2, test_y2 = load_test_data_nn('dataset/testset-levelb.tsv', 'dataset/labels-levelb.csv', word_dict,
                                             args.max_document_len)
        train_bert(train_x, train_y1, train_y2, test_x1, test_x2, test_y1, test_y2, args)
